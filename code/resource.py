...
def save_music_fallback(path, music: AudioClip):
    try:
        audio_data = music.m_AudioData
        if audio_data:
            fallback_path = path.replace('.ogg', '.fsb')
            queue_in.put((fallback_path, audio_data))
            print(f" 音频保存为原始格式: {fallback_path}")
            print(f"  提示: 可以使用其他工具转换 FSB 文件为 OGG")
        else:
            print(f"错误: 无法提取音频数据 {path}")
    except Exception as e:
        print(f"音频处理失败 {path}: {e}")


def save_music(path, music: AudioClip):
    if config.get("HAS_FSB5", False):
        try:
            fsb = FSB5(music.m_AudioData)
            rebuilt_sample = fsb.rebuild_sample(fsb.samples[0])
            queue_in.put((path, rebuilt_sample))
            print(f"使用 FSB5 处理音频: {path}")
        except Exception as e:
            print(f" FSB5 处理失败 {path}: {e}")

            save_music_fallback(path, music)
    else:
...
def main(apk_path):
    ...
    if config["music"]:
        try:
            ...
            config["HAS_FSB5"] = True
            print("FSB5 可用，将尝试使用")
        except ImportError as e:
            print(f"FSB5 不可用: {e}，将使用备用音频方案")
            config["HAS_FSB5"] = False
        except Exception as e:
            print(f"FSB5 初始化失败: {e}，将使用备用音频方案")
            config["HAS_FSB5"] = False
...
if __name__ == "__main__":
    # 安全地获取参数，如果没有参数就使用默认值
    path = sys.argv[1] if len(sys.argv) > 1 else "PhigrosApk/base.apk"
    main(path)